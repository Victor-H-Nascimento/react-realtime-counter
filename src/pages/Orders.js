...
const Orders = () => {
  ...
  function handleAddItem(itemSelected) {
    const itemIndex = order.items?.findIndex(
      (element) => element.product === itemSelected
    );
    let aux;
  ...
  function handleSend(e) {
    ...
    const nameValid =
      !!order.customer?.split(" ")[1] && order.customer?.split(" ")[1] !== "";

    if (!nameValid) setErrors([...errors, "Coloque nome e sobrenome"]);
    if (!order.customer) setErrors([...errors, "Colocar nome do cliente"]);
    else if (order.items.length === 0)
      setErrors([...errors, "Não tem nenhum pastel no pedido"]);
      ...
      <div className={style.right}>
        <div className={style.orders}>
          ...
          <div className={style.wrapper}>
            {newOrders.map((item, idx) => (
              <Order id={idx} order={item} status="queued" />
            ))}
          </div>
        </div>
        ...
        <div className={style.ready}>
          ...
          <div className={style.wrapper}>
            {newOrders.map((item, idx) => (
              <Order id={idx} order={item} status="ready" />
            ))}
          </div>
        </div>
      </div>
